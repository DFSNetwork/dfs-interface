...
export default {
    ...
    marketLists: {
      handler: function mlt(newVal, oldVal) {
        ...
        if (this.type === 'kline') {
          ...
          const arr = dealSymArr(newVal)
          this.coinList = arr;
          const arr2 = dealSymArr(this.dealFilter)
          this.filterCoinList = arr2;
        } else {
          this.searchArr = this.dealFilter;
        }
        this.handleSearch();
      },
      deep: true,
  ...
  methods: {
    handleDealFilter() {
      const dealFilter = JSON.parse(JSON.stringify(this.filterMkLists))
      this.marketLists.forEach(v => {
        if (!v.pid) {
          return
        }
        const dHas = dealFilter.find(vv => vv.mid === v.mid);
        if (!dHas) {
          return
        }
        dHas.pid = v.pid
